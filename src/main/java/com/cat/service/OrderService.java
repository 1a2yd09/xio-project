...
public class OrderService {
    ...
    public List<NormalBoard> getBoardList(CuttingSignal signal, List<WorkOrder> workOrders, BigDecimal wasteThreshold) {
        ...
        if (workOrders.isEmpty()) {
            ...
            // 在此处进行库存件的处理，因为没有工单队列中没有工单，代表上一个工单就是一批次中的最后一个工单，并且原料板材仍有剩余:
            ...
            // 此时板材列表中仅有各个工单的成品，此时考虑库存件，先比较长度，如果库存件比最后一个成品的长度小，表示库存件需要放在最后处理，
            // 考虑剩余宽度以及库存件长度，是否需要修边，是否可以复用245的进刀深度，
            // 如果库存件比最后一个成品的长度大，
            return normalBoards;
        }
        ...
        if (incompleteQuantity > 0 && destBoard.getWidth().compareTo(remainWidth) <= 0) {
            if (destBoard.getWidth().compareTo(BoardUtil.CLAMP_DEPTH) >= 0) {
                ...
                // 成品板
                NormalBoard extraProBoard = new NormalBoard(srcBoard.getHeight(), destBoard.getWidth(), destBoard.getLength(), destBoard.getMaterial(), order.getId() > 0 ? BoardCategory.PRODUCT : BoardCategory.STOCK, order.getId());
